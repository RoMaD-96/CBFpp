...
for (M in 1:length(par_seq)) {
  ...
  for (i in 1:nrow(grid_beta)) {
    obs_log_bf[i] <- round(bf_fun(
      delta_seq = delta_seq, y = y, N = N, y_0 = y_0,
  ...
  for (i in 1:nrow(grid_beta)) {
    pb$tick()
    norm_const <- marg_lik(
      delta = delta_seq, y = y, N = N, y_0 = y_0, N_0 = N_0,
      p = q, q = q, eta = grid_beta$eta_val[i], nu = grid_beta$nu_val[i]
    )
    post_theta[[i]] <- marg_post_theta(
      theta = theta_seq, y = y, N = N, y_0 = y_0, N_0 = N_0,
      p = q, q = q, eta = grid_beta$eta_val[i],
      nu = grid_beta$nu_val[i], norm_const
    )
    ...
    for (j in 1:n_rep) {
      ...
      bf_list[[i]][j] <- round(bf_fun(
        delta_seq = delta_seq, y = y_rep[[i]][[j]], N = N, y_0 = y_0,
        N_0 = N_0, p = p, q = q, eta_null = eta_null, nu_null = nu_null,
        eta_alt = grid_beta$eta_val[i], nu_alt = grid_beta$nu_val[i],
        log_val = TRUE
      ), 4)
