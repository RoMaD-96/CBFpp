...
gaussian_comp <- ggplot(results_df, aes(x = theta_dif, y = median, color = hpdi_val, group = hpdi_val)) +
  geom_point(size=2) +
  geom_line(size=1) +
  geom_errorbar(aes(ymin = first_q, ymax = third_q), width = 0.08, size = 0.9) +
  geom_text(aes(y = third_q, label = paste("(", eta, ",", nu, ")")), vjust = -0.8, hjust = -0.2, size = 3.5, angle = 45, check_overlap = FALSE) +
  ...
  theme(
    ...
    axis.text.x = element_text(size = 16, angle = 45, vjust = 1)) + # Adjust the angle and vertical alignment here
  labs(
  ...
  scale_y_continuous(breaks = seq(0, 1, length.out = 5), labels = scales::number_format(accuracy = 0.1), limits = c(0, max(results_df$third_q+0.5)), guide = guide_axis(check.overlap = TRUE))+
...
standard_dev_plot <- ggplot(df_long, aes(x = theta_dif, y = value, color = sd_type, shape = sd_type, group = sd_type)) +
  ...
  scale_x_continuous(breaks = seq(0, max(results_df$theta_dif), 0.4), 
