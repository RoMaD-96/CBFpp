...
process_datasets <- function() {
  ...
  for (file in files) {
    ...
    for (j in 1:length(hpdi)) {
      ...
      post_mu_unif <- theta_m_post(
        theta_seq = theta_seq, theta = theta,
        theta_0 = theta_0, st_dev = se, st_dev_0 = se_0,
        eta = 1, nu = 1
      )
      post_delta_unif <- delta_m_post(
        delta_seq = delta_seq[2:499], theta = theta,
        theta_0 = theta_0, st_dev = se, st_dev_0 = se_0,
        eta = 1, nu = 1
      )
      ...
      # RMAP prior
      post_mu_mix <- thetaposteriormix(
        theta = theta_seq, tr = theta,
        to = theta_0, sr = se, so = se_0,
        w = 0.5, m = 0, v = 100
      )
      post_omega_mix <- wposteriormix(
        w = delta_seq[2:499], tr = theta,
        to = theta_0, sr = se, so = se_0,
        x = 1, y = 1, m = 0, v = 100
      )
      ...
      if (optimal_model_index == "The model under the Null Hypothesis is optimal") {
        ...
        results[[file]][[j]] <- list(
          opt_index = optimal_model_index, eta = 1,
        ...
        results[[file]][[j]] <- list(
          opt_index = optimal_model_index, eta = grid_beta$eta_val[optimal_model_index],
...
correct_col_names <- c(
  "file", "opt_index", "eta", "nu", "theta_dif",
...
ggsave(
  filename = "gaussian_comp_mu_unknown.pdf", path = "Plots", plot = gaussian_comp,
...
df_long <- pivot_longer(results_df,
  cols = c(
    sd_mu_ref, sd_mu_alter,
...
ggsave(
  filename = "sd_gaussian_comp.pdf", path = "Plots", plot = standard_dev_plot,
...
df_long_mean <- pivot_longer(results_df,
  cols = c(
    mean_mu_ref, mean_mu_alter,
...
ggsave(
  filename = "mean_gaussian_comp.pdf", path = "Plots", plot = mean_plot,
  width = 15, height = 10, device = "pdf", dpi = 500, useDingbats = FALSE
)
...
plot_comb_mu <- ggarrange(standard_dev_plot, mean_plot,
  ncol = 2, nrow = 1,
  common.legend = TRUE
)
plot_comb_mu <- annotate_figure(plot_comb_mu,
  left = textGrob("Posterior Values", rot = 90, vjust = 0.8, gp = gpar(cex = 1.8)),
  bottom = textGrob(expression("Values of " * (mu[r] - mu[o])), vjust = -0.1, gp = gpar(cex = 1.8))
)


ggsave(
  filename = "combined_plot_gaussian.pdf", path = "Plots", plot = plot_comb_mu,
  width = 15, height = 10, device = "pdf", dpi = 500, useDingbats = FALSE
)
...
ggsave(
  filename = "gaussian_delta_plot.pdf", path = "Plots", plot = gaussian_delta_plot,
  width = 15, height = 8, device = "pdf", dpi = 500, useDingbats = FALSE
)
