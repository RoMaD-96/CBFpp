...
for (M in 1:length(par_seq)) {
  ...
  for (i in 1:nrow(grid_beta)) {
    obs_log_bf[i] <- round(bf_fun(
      delta = delta_seq, theta = theta, theta_0 = theta_0,
  ...
  for (i in 1:nrow(grid_beta)) {
    ...
    for (j in 1:n_rep) {
      ...
      bf_list[[i]][j] <- round(bf_fun(
        delta_seq = delta_seq, theta = theta_rep[[i]][[j]], theta_0 = theta_0,
        st_dev = sd_rep[[i]][[j]], st_dev_0 = se_0, eta_null = eta_null, nu_null = nu_null,
        eta_alt = grid_beta$eta_val[i], nu_alt = grid_beta$nu_val[i], log_val = TRUE
      ), 4)
    }
  }
