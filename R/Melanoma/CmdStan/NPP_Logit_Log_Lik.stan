  ...
  if ( dist <= 2 ) {
    ...
    for ( k in 2:K ) {
      ...
      target += a0s[k-1] * glm_lp(y[ start_idx[k]:end_idx[k] ],
                                  beta, 1.0, X[ start_idx[k]:end_idx[k], ],
                                  dist, link, offs[ start_idx[k]:end_idx[k] ]);
    }
  }
  else {
    ...
    for ( k in 2:K ) {
      ...
      // power prior
      target += a0s[k-1] * glm_lp(y[ start_idx[k]:end_idx[k] ],
                                  beta, dispersion[1], X[ start_idx[k]:end_idx[k], ],
                                  dist, link, offs[ start_idx[k]:end_idx[k] ]);
    }
  }
...
generated quantities {
  vector[N1] log_lik;
  {
    real phi = (dist > 2) ? dispersion[1] : 1.0;
    for (n in 1:N1) {
      real eta = dot_product(X[n], beta) + offs[n];
      // Use rep_vector to ensure eta is a vector
      real mu  = lp2mean(rep_vector(eta, 1), link)[1];
      if (dist == 1) {
        // Bernoulli
        real theta = (link == 3) ? eta : logit(mu);
        log_lik[n] = y[n]*theta - log1p_exp(theta);
      } else if (dist == 2) {
        // Poisson
        real theta = (link == 2) ? eta : log(mu);
        log_lik[n] = y[n]*theta - exp(theta) - lgamma(y[n]+1);
      } else if (dist == 3) {
        // Normal
        real theta = (link == 1) ? eta : mu;
        log_lik[n] = normal_lpdf(y[n] | theta, sqrt(phi));
      } else if (dist == 4) {
        // Gamma
        real tau   = inv(phi);
        real theta = (link == 4) ? eta : inv(mu);
        log_lik[n] = gamma_lpdf(y[n] | tau, tau * theta);
      } else if (dist == 5) {
        // Inverse Gaussian
        real tau     = inv(phi);
        real log_2pi = 1.837877066409345483560659;  // log(2*pi)
        real theta   = (link == 9) ? eta : inv_square(mu);
        log_lik[n] = 0.5 * ((log(tau) - log_2pi) - 3 * log(y[n])
                            - tau * square((y[n]*sqrt(theta)-1)*inv_sqrt(y[n])));
      }
    }
  }
}
