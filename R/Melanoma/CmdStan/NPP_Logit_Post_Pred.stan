...
generated quantities {
  ...
  // Compute linear predictor theta
  theta = X[ start_idx[1]:end_idx[1], ] * beta + offs[ start_idx[1]:end_idx[1] ];
  ...
  if (dist == 1) { // Bernoulli
    ...
    mu = exp(theta);
    for (n in 1:N1)
      y_rep[n] = poisson_rng(mu[n]);
  } else if (dist == 3) { // Normal
    if (link != 1)
      theta = lp2mean(theta, link);
    mu = theta;
    real sigma = sqrt(dispersion[1]);
    y_rep = normal_rng(mu, sigma);
  } else if (dist == 4) { // Gamma
    if (link != 4)
      theta = inv( lp2mean(theta, link) );
    mu = inv(theta);
    real tau = inv(dispersion[1]); // shape parameter
    for (n in 1:N1) {
      y_rep[n] = gamma_rng(tau, tau * mu[n]);
    }
  } else {
