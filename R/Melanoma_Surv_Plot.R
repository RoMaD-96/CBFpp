...
hd_surv <- survfit(Surv(historical_data$failtime,
                        historical_data$failind) ~ treatment, data = historical_data)
cd_surv <- survfit(Surv(current_data$failtime,
                        current_data$failind) ~ treatment, data = current_data)
...
plot_cd <- ggsurvplot_facet(cd_surv, data = current_data, combine = TRUE,
             facet.by = "trial_name",
             short.panel.labs = TRUE,
             pval = FALSE,
             conf.int = TRUE,
